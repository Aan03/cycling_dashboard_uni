...
def get_all_reports():
    all_reports = db.session.execute(
        db.select(Reports).order_by(desc(Reports.report_date),
...
def get_borough_reports(borough):
    if borough.lower() in boroughs_lower:
        borough_reports = db.session.execute(
            db.select(Reports).filter(func.lower(
                    Reports.report_borough) == borough.lower(
                    )).order_by(desc(
                                Reports.report_date), desc(
                                Reports.report_time))).scalars()
        reports_dump = reports_schema.dump(borough_reports)
        response_json = jsonify({"report": reports_dump})
        response_json.headers["Content-Type"] = "application/json"
        return make_response(response_json, 200)
    else:
        return make_response("404: The borough name is incorrect or"
...
rack_id_list = [
    element for nestedlist in rack_id_list for element in nestedlist]
rack_id_list_lower = []
...
def get_rack_id_reports(report_rack_id):
    if str(report_rack_id.lower()) in rack_id_list_lower:
        rack_id_reports = db.session.execute(
            db.select(Reports).filter((Reports.rack_id) == str(
                report_rack_id.upper())).order_by(desc(
                    Reports.report_date), desc(
                    Reports.report_time))).scalars()
...
def get_all_reports_for_user(username_get):
    ...
    if user_check:
        all_reports_by_user = Reports.query.filter_by(
            reporter_id=user_check.id).order_by(
            desc(Reports.report_date), desc(Reports.report_time))
        ...
        response = jsonify("404: A user with the username " + username_get +
                           " does not exist.")
...
def post_report():
    ...
    if user_check:
        ...
        response = jsonify("404: A user with the username " +
                           post_username + " does not exist.")
        return make_response(response, 404)
...
def new_user_api():
    ...
    if user_check:
        ...
        new_user = Users(username=new_user_username,
        ...
        if sha256_crypt.verify(put_password, user_check.password) == True:
            ...
            if report_check:
                if user_check.id == report_check.reporter_id:
                    ...
                    response = jsonify("The report with an id of "
                                       + str(put_report_id)
                                       + " does exist but under a different"
                                         " username.")
                    return make_response(response, 200)
            else:
                response = jsonify("404: A report with that ID"
                                   " does not exist.")
                return make_response(response, 404)
        else:
...
def delete_report(report_id):
    ...
    if user_check:
        if sha256_crypt.verify(delete_req_password,
                               user_check.password) == True:
            report_check = Reports.query.filter_by(
                id=delete_req_report_id).first()
            if report_check:
                if user_check.id == report_check.reporter_id:
                    ...
                    response = jsonify("The report with an id of " +
                                       str(delete_req_report_id) +
                                       " does exist"
                                       " but under a different username.")
                    return make_response(response, 200)
            else:
                response = jsonify("404: A report with that"
                                   " ID does not exist.")
                return make_response(response, 404)
        else:
            response = jsonify('Password received was incorrect.')
