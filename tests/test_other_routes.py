...
@pytest.mark.parametrize("test_input, expected",
                         [("password123", "Password changed successfully."),
                          ("wrong password",
...
@pytest.mark.parametrize("expected",
                         [("Manage/View Your Submitted Theft Reports:")])
...
def test_index_content(test_client):
    ...
    THEN check the response contains 'Sign up or login to submit and
    manage theft reports.'.
    """
    response = test_client.get('/')
...
def test_dash_statistics_content(test_client):
    ...
    THEN check the response contains the correct dash
    iframe.
    """
...
@pytest.mark.parametrize("test_input, expected",
                         [("RWG148175", "active theft report for RWG148175"),
...
def test_specific_reports_content(test_client, test_input, expected):
    ...
    THEN check that the response contains the active theft reports for
    the correct bike rack.
    """
...
def test_my_reports_logged_out_content(test_client):
    ...
    THEN check the response contains the message:
