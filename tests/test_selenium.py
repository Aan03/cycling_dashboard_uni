...
def test_sign_up_then_login(run_app_win, chrome_driver, flask_port, test_input):
    ...
    time.sleep(10)
    sign_up_nav = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,'''//*[@id="nav-sign_up"]''')))
    sign_up_nav.click()
    username_sign_up_entry = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,'''//*[@id="username"]''')))
    username_sign_up_entry.send_keys(test_input[0])
    password_sign_up_entry = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,'''//*[@id="password"]''')))
    ...
    chrome_driver.get_screenshot_as_file("screenshots/selenium_screenshots/logging_in.png")
    time.sleep(5)
    submit_sign_up = chrome_driver.find_element(By.XPATH, '''//*[@id="submit"]''')
    submit_sign_up.click()
    time.sleep(10)
...
def test_create_report(run_app_win, chrome_driver, flask_port, test_input, expected):
    ...
    THEN the report should only be submitted to the database if it passes validation
    ...
    chrome_driver.get(url)
    time.sleep(10)
    login_nav = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,'''//*[@id="nav-login"]''')))
    login_nav.click()
    time.sleep(10)
    username_login_entry = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,'''//*[@id="username"]''')))
    ...
    main_map = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,'''//*[@id="map"]/div[1]/div[3]/canvas''')))
    main_map.click()
...
def test_change_user_password(run_app_win, chrome_driver, flask_port, test_input, expected):
    ...
    chrome_driver.get_screenshot_as_file("screenshots/selenium_screenshots/changing_password.png")
...
def test_all_check_boxes_map(run_app_win, chrome_driver, flask_port):
    ...
    WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,'''//*[@id="map"]/div[1]/div[3]/canvas''')))

    covered_check_box = chrome_driver.find_element(By.XPATH, '//*[@id="covered_box"]')
    secured_check_box = chrome_driver.find_element(By.XPATH, '//*[@id="secured_box"]')
    locker_check_box = chrome_driver.find_element(By.XPATH, '//*[@id="locker_box"]')

    covered_check_box.click()
    time.sleep(5)
    chrome_driver.get_screenshot_as_file("screenshots/selenium_screenshots/covered_filter.png")
    secured_check_box.click()
    time.sleep(5)
    chrome_driver.get_screenshot_as_file("screenshots/selenium_screenshots/secured_filter.png")
    locker_check_box.click()
    time.sleep(5)
    chrome_driver.get_screenshot_as_file("screenshots/selenium_screenshots/locker_filter.png")


@pytest.mark.parametrize("test_input, expected", [("my_reports", "Please log in to access this page."),
                                        ("account", "Please log in to access this page.")])
def test_login_warning(run_app_win, chrome_driver, flask_port, test_input, expected):
    """
    GIVE a running app
    WHEN an anonymous (not logged in) user goes to the "my reports" or "manage account" page 
         (which are only for logged in users)
...
def test_download_data_empty(run_app_win, chrome_driver, flask_port):
    ...
    AND chooses to download the reports from a borough with no data/reports available
    THEN a flask flash message will appear telling the user that there is no data 
    ...
    time.sleep(10)
    flash_no_data = WebDriverWait(chrome_driver,20).until(EC.visibility_of_element_located((By.XPATH,
                                                                        '''/html/body/ul/li''')))
    ...
    assert WebDriverWait(chrome_driver,20).until(EC.
                                                 visibility_of_element_located((By.XPATH,
                                                        '''/html/body/div/h2'''))).text == "Download Reports"
...
def test_download_data_page(run_app_win, chrome_driver, flask_port):
    ...
    chrome_driver.get(url)
    time.sleep(10)
    nav_download_reports_button = WebDriverWait(chrome_driver, timeout=15).until(
        lambda d: d.find_element(By.XPATH, '//*[@id="nav-download_reports"]')
    )
    time.sleep(5)
    #nav_download_reports_button = chrome_driver.find_element(By.XPATH, "/html/body/div/form/button")
...
def test_api_instructions_page_selected(run_app_win, chrome_driver, flask_port):
   ...
   # nav_api_button = WebDriverWait(chrome_driver, timeout=3).until(
   #     lambda d: d.find_element(By.XPATH, '//*[@id="nav-api"]')
   # )

    nav_api_button = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,
                                                                        '''//*[@id="nav-api"]''')))
...
def test_login_page_selected(run_app_win, chrome_driver, flask_port):
    ...
    #nav_login_button = WebDriverWait(chrome_driver,20).until(EC.element_to_be_clickable((By.XPATH,
     #                                                                   '''//*[@id="nav-login"]''')))
